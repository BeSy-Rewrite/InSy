 ...
 */
function handleKeyEvent(evt, statisticsScreen, debugCallback, buttons) {
    // Für Message-Events: evt.data.key statt evt.key
    const key = evt.key || (evt.data && evt.data.key);
...
export function setupEventListeners(cnv, joystick, slider, buttons, movementController, statisticsScreen, debugCallback) {
    ...
    window.addEventListener("keydown", (evt) => {
        handleKeyEvent(evt, statisticsScreen, debugCallback, buttons);
    });

    window.addEventListener("message", (evt) => {
        if (evt.data && evt.data.type === "keydown") {
            handleKeyEvent(evt, statisticsScreen, debugCallback, buttons);
        }
    });
}
